...
def request(url, data, timeout=30):
    ...
    :return:
    '''
    req = urllib.Request(url, data=data, headers={'Content-Type': 'application/xml'})
    try:
        result = urllib.urlopen(req, timeout=timeout).read()
        return result
...
class WxPay(object):
    ...
    def get_pay_info(self):
        ...
        response = request(url=self.url, data=xml_data.encode())
        print(response)
        if response:
            return response
